...
#
                ...
                if E > C:
                    mirande2['c'].iloc[0] = mirande2['c'].iloc[1] - ((suite * (local_max[fc] - local_max[fa])))
                    mirande2['c'].iloc[3] = mirande2['c'].iloc[2] + ((suite * (local_max[fg] - local_max[fe])))
                if E < C:
                    mirande2['c'].iloc[0] = mirande2['c'].iloc[1] + ((suite * (local_max[fc] - local_max[fa])))
                    mirande2['c'].iloc[3] = mirande2['c'].iloc[2] - ((suite * (local_max[fg] - local_max[fe])))
                if E == C:
                    mirande2['c'].iloc[0] = df['c'].iloc[local_max[fc]]
                    mirande2['c'].iloc[3] = df['c'].iloc[local_max[fe]]
                ...
                if (C - B) < (C - D) and (C - B) < (E - D) and (E- F) < (E - D) and (E - F) < (C - D) and B > D and F > D  and B < C and F < E and A >= mirande2['c'].iloc[0] and G >= mirande2['c'].iloc[3] and verif == 0 :
                    J = line_intersection((AJ, BJ), (CJ, DJ))
                    I = line_intersection((AI, BI), (CI, DI))
                    moyenne_epaule1 = ((I[1] - B) + (C - B)) / 2
                    moyenne_epaule2 = ((E - F) + (J[1] - F)) / 2
                    ...
                    if I[1] > B and J[1] > F and moyenne_epaule1 <= moyenne_tete/2 and moyenne_epaule2 <= moyenne_tete/2 and moyenne_epaule1 >= moyenne_tete/4 and moyenne_epaule2 >= moyenne_tete/4:
                        ...
                        file = open('/home/yumin/Desktop/admatv2/btest/iete/compteur_images.txt', 'r')
                        ...
                        file = open('/home/yumin/Desktop/admatv2/btest/iete/compteur_images.txt', 'w')
                        compteur_nombre_image = compteur_nombre_image + 1
                        file.write(f'{compteur_nombre_image}')
                        file.close()
                        plt.savefig(f'/home/yumin/Desktop/admatv2/btest/iete/images/figure_{compteur_nombre_image}_r1.png')
...
#
                        ...
                        while g !=1:
                            if df['c'].iloc[local_max[fg] + compteur2] < F:
                                resultat = "PERDANT "
                                file = open('/home/yumin/Desktop/admatv2/btest/iete/compteur_perdant.txt', 'r')
                                compteur_nombre_perdant = int(file.read())
                                file.close()
                                file = open('/home/yumin/Desktop/admatv2/btest/iete/compteur_perdant.txt', 'w')
                                compteur_nombre_perdant = compteur_nombre_perdant + 1
                                file.write(f'{compteur_nombre_perdant}')
                                file.close()
                                file.close()
                                g = 1
                            if df['c'].iloc[local_max[fg] + compteur2] >= J[1] + (moyenne_tete)/2:
                                resultat = "GAGNANT À +50%"
                                file = open('/home/yumin/Desktop/admatv2/btest/iete/compteur_gagnant.txt', 'r')
                                compteur_nombre_gagnant = int(file.read())
                                file.close()
                                file = open('/home/yumin/Desktop/admatv2/btest/iete/compteur_gagnant.txt', 'w')
                                compteur_nombre_gagnant = compteur_nombre_gagnant + 1
                                file.write(f'{compteur_nombre_gagnant}')
                                file.close()
                                file.close()
                                g = 1
                            if int(round(J[0], 0)) + compteur2 == point_max:
                                g = 1
...
#
                        ...
                        plt.savefig(
                            f'/home/yumin/Desktop/admatv2/btest/iete/images/figure_{compteur_nombre_image}_r3.png')

...
#
                ...
                if (C - B) < (C - D) and (C - B) < (E - D) and (E- F) < (E - D) and (E - F) < (C - D) and B > D and F > D  and B < C and F < E and A >= mirande2['c'].iloc[0] and verif == 0 :
                    ...
                    if accept == True:
                        ...
                        #------------------ determinage de la place de la premiere cloture apres ligne de coup ----------------#
                        if I[1] > B and J[1] > F and moyenne_epaule1 <= moyenne_tete/2 and moyenne_epaule2 <= moyenne_tete/2 and moyenne_epaule1 >= moyenne_tete/4 and moyenne_epaule2 >= moyenne_tete/4 and df['c'].values[place_pc] <= J[1] + (moyenne_tete) / 8 and df['c'].values[place_pc] >= J[1]  and G >= J[1] and place_pc <= J[0] and place_pc < local_max[fg] and place_pc > local_min[ff] and accept == True :#and df['rsi'].values[place_pc] > df['rsi'].values[place_pc-1]  :#and C < E and moyenne_epaule1 < moyenne_epaule2 and df['v'].values[place_pc-1] <= (df['v'].values[place_pc]) - ((df['v'].values[place_pc]*25)/100):#  and macd_croisee == True: #and essoufflement==True :# and (F-D) > (D-B) - (((D-B)*25)/100) and (F-D) < (D-B) + (((D-B)*25)/100) :# :

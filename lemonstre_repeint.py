...
def Finder_iete(time1, time_name1, start1):
    ...
    while i != 1:
        ...
        with my_lock:
            ...
            api_url_OHLC2 = f'https://api.polygon.io/v2/aggs/ticker/{ticker}/range/{time1}/{time_name1}/{start1}/{end}?adjusted=true&limit=50000&apiKey={api_key}'
            #----- recuperations des donnees polygons.io -----#
                ...
                if (C - B) < (C - D) and (C - B) < (E - D) and (E- F) < (E - D) and (E - F) < (C - D) and B > D and F > D  and B < C and F < E and A >= mirande2['c'].iloc[0] and verif == 0 :
                    ...
                    if accept == True:
                        ...
                        if I[1] > B and J[1] > F and moyenne_epaule1 <= moyenne_tete/2 and moyenne_epaule2 <= moyenne_tete/2 and moyenne_epaule1 >= moyenne_tete/4 and moyenne_epaule2 >= moyenne_tete/4 and df['h'].iloc[round(J[0])] > J[1] and accept == True:
                            ...
                            plt.title(f'IETE : {compteur} | {nom} | {time1} {time_name1} | {compteur_nombre_image+1}', fontweight="bold", color='black')
...
##
                            ## -----------------------lire et connaitre nom de image et enregistrer image--------------------------#
#
                            ##plt.savefig(f'/home/yumin/Desktop/admatv2/btest/iete/images/figure_{compteur_nombre_image}_r2.png')
#
                            ## -----------------------lire et connaitre nom de image et enregistrer image--------------------------#
#
                            #fig = plt.figure(figsize=(10, 7))
...
#
                            ...
                            while g !=1:
                                if df['c'].iloc[round(J[0]) + compteur2] < F and fait_50 ==0:
                                    resultat = "PERDANT "
                                    ...
                                    gain_cumulé = gain_cumulé - round((((J[1] - df['c'].iloc[round(J[0]) + compteur2] )*100)/J[1]),2)
                                    ...
                                    file.close()
                                    file = open('df.txt', 'w')
                                    valeur = (J[1])-(df['c'].iloc[round(J[0]) + compteur2])
                                    df_gain = df_gain.replace('%df_gain%',f'-{round((((valeur )*100)/J[1]),2)}\n%df_gain%')
                                    file.write(f'{df_gain}')
                                    file.close()
                                    file.close()

                                    g = 1
                                if (df['h'].iloc[round(J[0]) + compteur2] >= J[1] + (moyenne_tete)/4) and fait_25  == 0: # !!! MODIFIER NOM OBJECTIF EN HAUT !!!!
                                    fait_25 = 1
                                ...
                                if (df['h'].iloc[round(J[0]) + compteur2] >= J[1] + (moyenne_tete)/2) and fait_50 == 0: # !!! MODIFIER NOM OBJECTIF EN HAUT !!!!
                                    fait_50 = 1
                                    ...
                                    file.close()
                                    file = open('gain_cumulé.txt', 'w')
                                    gain_cumulé = gain_cumulé + round(((((df['h'].iloc[round(J[0]) + compteur2]) - (J[1]))*100)/J[1]),2)
                                    file.write(f'{round(gain_cumulé,2)}')
                                    file.close()
                                    file.close()
                                    ...
                                    file.close()
                                    file = open('df.txt', 'w')
                                    valeur2 = (df['h'].iloc[round(J[0]) + compteur2])-(J[1])
                                    df_gain = df_gain.replace('%df_gain%', f'{round((((valeur2)*100)/J[1]),2)}\n%df_gain%')
                                    file.write(f'{df_gain}')
                                    file.close()
                                    file.close()
                                ...
                                if (df['h'].iloc[round(J[0]) + compteur2] >= J[1] + (((moyenne_tete)/4) + ((moyenne_tete)/2))) and fait_75 == 0: # !!! MODIFIER NOM OBJECTIF EN HAUT !!!!
                                    fait_75 = 1
                                    ...
                                    file.close()
                                    file.close()
                                if (df['h'].iloc[round(J[0]) + compteur2] >= J[1] + (moyenne_tete)) and fait_100 == 0: # !!! MODIFIER NOM OBJECTIF EN HAUT !!!!
                                    fait_100 = 1
...
#all = ['X:ADAUSD','X:AVAXUSD','X:BTCUSD','X:CROUSD','X:DOGEUSD','X:DOTUSD','X:EGLDUSD','X:EOSUSD','X:ETHUSD','X:FTMUSD','X:GALAUSD','X:LINKUSD','X:MANAUSD','X:MATICUSD','X:SANDUSD','X:SOLUSD','X:TRXUSD','X:UNIUSD','X:XRPUSD','X:XTZUSD','X:ATOMUSD','X:CROUSD','X:AXSUSD','X:YFIUSD','ATVI','ADBE','AMD','ABNB','MSFT','GOOG','AMZN','AEP','AMGN','ADI','ANSS','AAPL','AMAT','ADSK','BIIB','EBAY','KO','META','TSLA','NVDA','DIS','PEP','NVO','TM','MCD','ABT','CSCO','NEE','WFC','NKE','UPS','CRM','BMY','MS','TXN','PM','TTE','HSBC','IBM','DE','CAT','BLK','ADP','ALGN','ASML','TEAM','BIDU','BKNG','AVGO','CDNS','CHTR','CTAS','CTSH','CMCSA','CPRT','COST','CRWD','CSX','DDOG','DXCM','DOCU','DLTR','KDP','EA','EXC','FAST','FISV','FTNT','GILD','HON','KLAC','IDXX','ILMN','INTC','INTU','ISRG','JD','KHC','LRCX','LULU','MAR','MRVL','MTCH','MELI','MCHP','MV','MRNA','MDLZ','MNST','NTES','NXPI','ORLY','OKTA','PCAR','PANW','PAYX','PYPL','PTON','PDD','QCOM','ROST','SGEN','SIRI','SWKS','SPLK','SBUX','SNPS','TMUS','WBA','XEL','ZM']
all=['ADN','ADNT','ADOC','ADOCR','ADOCW','ADP','ADPT','ADRA','ADRT','ADSE','ADSEW','ADSK','ADT','ADTH','ADTHW','ADTN','ADTX','ADUS','ADV','ADVM','ADX','ADXN','AE','AEAC','AEE','AEF','AEFC','AEG','AEI','AEIS','AEL','AEM','AEMD','AENZ','AEO','AEP','AEPPZ','AER','AERC','AERI','AES','AESC','AESC','AEVA','AESE','AEY','AEYE','AEZS','AFAC','AFACW','AFAQ','AFAQU','AFAQW','AFB','AFBI','AFCG','AFG','AFGB','AFGC','AFGD','AFGE','AFI','AFIB','AFL','AFMD','AFRM','AFT','AFTR','AFYA','AG','AGAC','AGBA','AGBAR','AGBAW','AGCB','AGCO','AGD','AGE','AGEN','AGFS','AGFY','AGGR','AGGRU','AGI','AGIL','AGILW','AGIO','AGL','AGLE','AGM','AGMH','AGNC','AGNCM','AGNCN','AGNCO','AGNCP','AGO','AGR','AGRI','AGRIW','AGRO','AGRX','AGS','AGTC','AGTI','AGX','AGYS','AHCO','AHG','AHH','AHI','AHPA','AHPAU','AHPAW','AHPI','AHRN','AHRNU','AHT','AI','AIB','AIC','AIF','AIG','AIH','AIHS','AIKI','AIM','AIMAU','AIMC','AIN','AINC','AINV','AIO','AIO','AIP','AIR','AIRC','AIRG','AIRI','AIRS','AIRT','AIRTP','AIT','AIU','AIV','AIZ','AIZN','AJG','AJRD','AJX','AJXA','AKA','AKAM','AKAN','AKBA','AKIC','AKR','AKRO','AKTS','AKTX','AKU','AKUS','AKYA','ALB','ALBO','ALC','ALCC','ALCO','ALDX','ALE','ALEC','ALEX','ALF','ALFIW','ALG','ALGM','ALGN','ALGS','ALGT','ALHC','ALIT','ALJJ','ALK','ALKS','ALLE','ALLG','ALLK','ALLO','ALLR','ALLT','ALLY','ALNA','ALNY','ALOR','ALORU','ALORW','ALOT','ALPA','ALPAW','ALPN','ALPP','ALR','ALRM','ALRN','ALRS','ALSA','ALSAR','ALSAW','ALSN','ALT','ALTG','ALTO','ALTR','ALTU','ALTUU','ALTUW','ALV','ALVR','ALX','ALXO','ALYA','ALZN','AM','AMAL','AMAM','AMAO','AMAOW','AMAT','AMBA','AMBC','AMBO','AMBP','AMC','AMCI','AMCIW','AMCR','AMCX','AMD','AME','AMED','AMEH','AMG','AMGN','AMH','AMK','AMKR','AMLX','AMN','AMNB','AMOT','AMOV','AMP','AMPE','AMPG','AMPGW','AMPH','AMPI','AMPL','AMPS','AMPY','AMR','AMRC','AMRK','AMRN','AMRS','AMRX','AMS','AMSC','AMSF','AMST','AMSWA','AMT','AMTB','AMTD','AMTI','AMTX','AMWD','AMWL','AMX','AMYT','AMZN','AN','ANAB','ANAC','ANAT','ANDE','ANEB','ANET','ANF','ANGH','ANDE','ANEB','ANET','ANF','ANGH','ANGHW','ANGI','ANGN','ANGO','ANIK','ANIP','ANIX','ANPC','ANSS','ANTE','ANTM','ANTX','ANVS','ANY','ANZU','ANZUU','ANZUW','AOD','AOGO','AOGOW','AOMR','AON','AORT','AOS','AOSL','AOUT','AP','APA','APAC','APACW','APAM','APCA','APCX','APCXW','APD','APDN','APEI','APEN','APG','APGB','APH','API','APLD','APLE','APLS','APLT','APM','APMI','APMIU','APMIW','APN','APO','APOG','APP','APPF','APPH','APPHW','APPN','APPS','APRE','APRN','APSG','APT','APTMU','APTMW','APTO','APTS','APTV','APTX','APVO','APWC','APXI','APXIU','APXIW','APYX','AQB','AQMS','AQN','AQNA','AQNB','AQNU','AQST','AQUA','AR','ARAV','ARAY','ARBE','ARBEW','ARBG','ARBGU','ARBGW','ARBK','ARBKL','ARC','ARCB','ARCC','ARCE','ARCH','ARCK','ARCKU','ARCKW','ARCO','ARCT','ARDC','ARDS','ARDX','ARE','AREB','AREBW','AREC','AREN','ARES','ARGD','ARGO','ARGU','ARGUU','ARGUW','ARGX','ARHS','ARI','ARIS','ARIZ','ARIZR','ARIZU','ARIZW','ARKO','ARKOW','ARKR','ARL','ARLO','ARLP','ARMK','ARMP','ARNC','AROC','AROW','ARQQ','ARQQW','ARQT','ARR','ARRW','ARRWU','ARRWW','ARRY','ARTE','ARTEW','ARTL','ARTLW','ARTNA','ARTW','ARVL','ARVN','ARW','ARWR','ARYD','ARYE','ASA','ASAI','ASAN','ASAQ','ASAX','ASB','ASC','ASCA','ASCAR','ASCBU','ASG','ASGI','ASGN','ASH','ASIX','ASLE','ASLN','ASM','ASMB','ASML','ASND','ASNS','ASO','ASPA','ASPAU','ASPAW','ASPC','ASPCU','ASPCW','ASPN','ASPS','ASPU','ASR','ASRT','ASRV','ASTC','ASTE','ASTL','ASTLW','ASTR','ASTS','ASTSW','ASUR','ASX','ASXC','ASYS','ASZ','ATA','ATAI','ATAK','ATAKR','ATAKU','ATAKW','ATAQ','ATAX','ATC','ATCO','ATCOL','ATCX','ATEC','ATEK','ATEN','ATER','ATEX','ATGE','ATHE','ATHM','ATHX','ATI','ATIF','ATIP','ATKR','ATLC','ATLCL','ATLCP','ATLO','ATNF','ATNFW','ATNI','ATNM','ATNX','ATO','ATOM','ATOS','ATR','ATRA','ATRC','ATRI','ATNFW','ATNI','ATNM','ATNX','ATO','ATOM','ATOS','ATR','ATRA','ATRC','ATRI','ATRO','ATRS','ATSG','ATTO','ATUS','ATVC','ATVCW','ATVI','ATXI','ATXS','ATY','AU','AUB','AUBAP','AUBN','AUBN','AUBAP','AUD','AUDC','AUGX','AUID','AUMN','AUPH','AUR','AURA','AURC','AURCU','AURCW','AUROW','AUS','AUST','AUTL','AUTO','AUUD','AUUDW','AUVI','AUVIP','AUY','AUVIP','AUY','AVA','AVAC','AVACW','AVAH','AVAL','AVAN','AVAV','AVB','AVCO','AVCT','AVCTW','AVD','AVDL','AVDX','AVEO','AVGO','AVGOP','AVGR','AVHI','AVHIW','AVID','AVIR','AVK','AVLR','AVNS','AVNT','AVNW','AVO','AVPT','AVPTW','AVRO','AVT','AVTE','AVTR','AVTX','AVXL','AVY','AVYA','AWF','AWH','AWI','AWK','AWP','AWR','AWRE','AWX','AX','AXAC','AXDX','AXGN','AXH','AXL','AXLA','AXNX','AXNX','AXNX','AXNX','AXON','AXP','AXR','AXS','AXSM','AXTA','AXTI','AXU','AY','AYI','AYLA','AYRO','AYTU','AYX','AZ','AZEK','AZN','AZO','AZPN','AZRE','AZTA','AZUL','AZYO','AZZ','B','BA','BABA','BAC','BACA','BAFN','BAH','BALL','BAK','BALY','BAM','BAMH','BAMI','BAMR','BANC','BAND','BANF','BANFP','BANR','BANX','BAOS','BAP','BARK','BASE','BATL','BATRA','BATRK','BAX','BB','BBAI','BBAR','BBBY','BBCP','BBD','BBDC','BBDO','BBGI','BBI','BBIG','BBIGV','BBIO','BBLG','BBLGW','BBLN','BBN','BBQ','BBSI','BBU','BBUC','BBVA','BBW','BBY','BBWI','BC','BCAB','BCAC','BCACU','BCACW','BCAT','BCBP','BCC','BCDA','BCE','BCEL','BCH','BCLI','BCML','BCO','BCOR','BCOV','BCOW','BCPC','BCRX','BCS','BCSA','BCSAU','BCSAW','BCSF','BCTX','BCTXW','BCV','BCX','BCYC','BDC','BDJ','BDL','BDN','BDR','BDSX','BDTX','BDX','BDXB','BEAM','BE','BEAT','BECN','BEDU','BEEM','BEEMW','BEKE','BELFA','BELFB','BEN','BENE','BENER','BENEW','BEP','BFAM','BFC','BFH','BFI','BFIIW','BFIN','BFK','BFLY','BFRI','BFRIW','BFS','BEPC','BEPH','BEPI','BERY','BEST','BFAC','BFST','BFZ','BG','BGB','BGCP','BGFV','BGH','BGI','BGNE','BGR','BGRY','BGRYW','BGS','BGSF','BGSX','BGT','BGX','BGXX','BGY','BH','BHACU','BHACW','BHAT','BHB','BHC','BHE','BHF','BHFAL','BHFAM','BHFAN','BHFAO','BHFAP','BHG','BHIL','BHK','BHLB','BHR','BHP','BHSE','BHSEU','BHSEW','BHV','BHVN','BIDU','BIG','BIGC','BIGZ','BIIB','BILI','BILL','BIMI','BIO','BIOC','BIOL','BIOR','BIOS','BIOSU','BIOSW','BIOT','BIOTU','BIOTW','BIOX','BIP','BIPC','BIPH','BIPI','BIRD','BIT','BITE','BITF','BJ','BJDX','BJRI','BK','BKCC','BKD','BKE','BKEP','BKEPP','BKH','BKI','BKKT','BKN','BKNG','BKR','BKSC','BKSY','BKT','BKTI','BKU','BKYI','BL','BLBD','BLBX','BLCM','BLCM','BLCO','BLCT','BLD','BLDE','BLDEW','BLDP','BLDR','BLE','BLEU','BLEUR','BLEUU','BLEUW','BLFS','BLFY','BLI','BLIN','BLK','BLKB','BLMN','BLND','BLNG','BLNGU','BLNGW','BLNK','BLNKW','BLPH','BLRX','BLSA','BLTE','BLTS','BLTSU','BLTSW','BLU','BLUA','BLUE','BLW','BLX','BLZE','BMA','BMAC','BMAQ','BMAQR','BMAQU','BMAQW','BMBL','BME','BMEA','BMEZ','BMI','BMO','BMRA','BMRC','BMRN','BMTX','BMY','BNED','BNFT','BNGO','BNGOW','BNIX','BNIXR','BNIXW','BNNR','BNL','BNNRU','BNNRW','BNOX','BNR','BNS','BNSO','BNTC','BNTX','BNY','BOAC','BOAS','BOC','BOCN','BOCNW','BODY','BOE','BOH','BOKF','BOLT','BON','BOOM','BOOT','BORR','BOSC','BOTJ','BOWL','BOX','BOXD','BOXL','BP','BPAC','BPACW','BPMC','BPOP','BPOPM','BPRN','BPT','BPTH','BPTS','BPYPM','BPYPN','BPYPO','BPYPP','BQ','BR','BRAC','BRAG','BRBR','BRBS','BRC','BRCC','BRCN','BRDG','BRDS','BREZ','BREZR','BREZW','BRFH','BRFS','BRG','BRID','BRIV','BRIVU','BRIVW','BRKH','BRKHW','BRKL','BRKR','BRLI','BRLIR','BRLIW','BRLT','BRMK','BRN','BRO','BROG','BROGW','BROS','BRP','BRPM','BRPMU','BRPMW','BRQS','BRSP','BRT','BRTX','BRW','BRX','BRY','BRZE','BSAC','BSAQ','BSBK','BSBR','BSET','BSFC','BSGA','BSGAR','BSGAU','BSGM','BSIG','BSKY','BSKYU','BSKYW','BSL','BSM','BSMX','BSQR','BSRR','BST','BSTZ','BSVN','BSX','BSY','BTA','BTU','BTWN','BTWNU','BTWNW','BTX','BTZ','BUD','BUI','BUR','BURL','BUSE','BV','BVH','BVN','BVS','BVXV','BTAI','BTB','BTBD','BTBDW','BTBT','BTCM','BTCS','BTCY','BTG','BTI','BTO','BTN','BTOG','BTT','BTTR','BTTX','BW','BWA','BWAC','BWACU','BWACW']
